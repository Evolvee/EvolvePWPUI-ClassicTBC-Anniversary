...
for pFrame in PartyFrame.PartyMemberFramePool:EnumerateActive() do
    ...
    local healthText = pFrame.PartyMemberOverlay.Name:GetParent():CreateFontString(nil, "OVERLAY", "TextStatusBarText")
    Mixin(TextStatusBarMixin, pFrame.HealthBar)
    pFrame.HealthBar:SetBarText(healthText)
    healthText:SetPoint("CENTER", 17, 4)

    local manaText = pFrame.PartyMemberOverlay.Name:GetParent():CreateFontString(nil, "OVERLAY", "TextStatusBarText")
    Mixin(TextStatusBarMixin, pFrame.ManaBar)
    pFrame.ManaBar:SetBarText(manaText)
    manaText:SetPoint("CENTER", 16, -10)

    pFrame.HealthBar.TextString:SetFont("Fonts\\FRIZQT__.TTF", 15, "THICKOUTLINE")
    pFrame.ManaBar.TextString:SetFont("Fonts\\FRIZQT__.TTF", 9, "THICKOUTLINE")

    pFrame.PartyMemberOverlay.Name:Hide()
    pFrame.PartyMemberOverlay.PVPIcon:SetAlpha(0)
    
    hooksecurefunc(pFrame.portrait, "SetVertexColor", function(self, r, g, b)
...
local function Evolve_UpdateTextString(self, textString, value)
    if not textString then return end
    
    local displayValue = self.finalValue or value
    
    if displayValue and displayValue > 0 then
        textString:SetText(displayValue)
        textString:Show()
    else
        textString:Hide()
    end
...
hooksecurefunc(PlayerFrameHealthBar, "UpdateTextStringWithValues", Evolve_UpdateTextString)
hooksecurefunc(PlayerFrameManaBar, "UpdateTextStringWithValues", Evolve_UpdateTextString)

hooksecurefunc(TargetFrameHealthBar, "UpdateTextStringWithValues", Evolve_UpdateTextString)
hooksecurefunc(TargetFrameManaBar, "UpdateTextStringWithValues", Evolve_UpdateTextString)

hooksecurefunc(FocusFrameHealthBar, "UpdateTextStringWithValues", Evolve_UpdateTextString)
hooksecurefunc(FocusFrameManaBar, "UpdateTextStringWithValues", Evolve_UpdateTextString)

hooksecurefunc(PetFrameHealthBar, "UpdateTextStringWithValues", Evolve_UpdateTextString)
hooksecurefunc(PetFrameManaBar, "UpdateTextStringWithValues", Evolve_UpdateTextString)

for pFrame in PartyFrame.PartyMemberFramePool:EnumerateActive() do
    hooksecurefunc(pFrame.HealthBar, "UpdateTextStringWithValues", Evolve_UpdateTextString)
    hooksecurefunc(pFrame.ManaBar, "UpdateTextStringWithValues", function(self, textString, value)
        if not textString then return end
        
        if self.powerType and self.powerType ~= 0 then
            textString:Hide()
        else
            Evolve_UpdateTextString(self, textString, value)
        end
    end)
end
