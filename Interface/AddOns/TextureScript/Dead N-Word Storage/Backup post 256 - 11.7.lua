...
--DEBUFF_TYPE_NONE_COLOR:SetRGB(0.80, 0.00, 0.00)
--DEBUFF_TYPE_MAGIC_COLOR:SetRGB(0.20, 0.60, 1.00)
--DEBUFF_TYPE_CURSE_COLOR:SetRGB(0.60, 0.00, 1.00)
--DEBUFF_TYPE_DISEASE_COLOR:SetRGB(0.60, 0.40, 0.00)
--DEBUFF_TYPE_POISON_COLOR:SetRGB(0.00, 0.60, 0.00)
local myDebuffColors = {
...
hooksecurefunc(AuraUtil, "SetAuraBorderColor", function(borderRegion, dispelType)
    local c = myDebuffColors[dispelType or "None"] or myDebuffColors.None
...
-- 2.5.5+ patch: the health bar border is no longer a "border" child frame
-- (HealthBarsContainer.border:GetRegions() is gone). It's now the texture
-- healthBar.bgTexture, and Blizzard's UnitFrame:UpdateAnchors() re-anchors
-- the containers and resets bgTexture on every options/CVar update.
-- So all custom anchoring + border texturing lives here and gets re-applied
-- via hooksecurefunc(frame, "UpdateAnchors", ...).
local CastBarVisuals -- forward declaration
local function EvolveUI_UpdateAnchors(self)
    ...
    -- Leave the HealthBarsContainer and CastBarsContainer where Blizzard puts
    -- them (cast bar at the bottom, health above it). Do NOT move the border
    -- (bgTexture) either -- Blizzard centers it on the container at a fixed size.
    --
    -- Instead, re-inset the HEALTH BAR inside the container so it sits centered
    -- within the border. Blizzard's classic health bar is inset +3.5 left /
    -- -20.75 right, which shoves the bar hard to the left inside our border art.
    hBar:ClearAllPoints()
    hBar:SetPoint("TOPLEFT", hb, "TOPLEFT", 4, 0)
    hBar:SetPoint("BOTTOMRIGHT", hb, "BOTTOMRIGHT", -4, 0)

    -- Restore the old dark background behind the health bar so depleted health
    -- shows a shadowy fill instead of full transparency. Created once per bar,
    -- SetAllPoints follows the bar as it re-insets. BACKGROUND layer keeps it
    -- behind the colored health fill.
...
-- Cosmetic-only cast bar handling. Positioning/sizing is left entirely to
-- Blizzard (moving/clearing the bar's points made it collapse or fall off the
-- bottom of the plate). We only show the spell text and hide friendly casts.
function CastBarVisuals(self)
    if not self or self:IsForbidden() then return end
    local container = self:GetParent()
    local unitFrame = container and container:GetParent() -- castBar -> CastBarsContainer -> UnitFrame
    if not unitFrame then return end

    if self.Text then
        -- nudge the spell text up by 1 (absolute set so it can't compound
        -- across casts). Blizzard's default: classic = y -1, non-classic = y 0.
...
-- Fix: install the recheck OnUpdate once. HandleNewNameplate :Show()s the frame
-- when it queues an unresolved-name unit, and plateOnUpdateFrame :Hide()s it when
-- the queue empties. Previously this OnUpdate was only set in arena/pvp, so
-- unresolved plates in the open world/dungeons never got re-processed.
...
local function AddPlates(unit)
    ...
    if not np[nameplate] then
        ...
        nameplate:HookScript("OnEvent", function(self, event)
            if event == "PLAYER_TARGET_CHANGED" then
                -- border swap (target highlight / tremor / default) now lives
                -- in EvolveUI_UpdateAnchors via healthBar.bgTexture
    ...
    -- 2.5.5+ patch: cast bar moved from UnitFrame.castBar to
    -- UnitFrame.CastBarsContainer.castBar (parent chain changed too)
    if not frame.EvolveUI_Hooked then
        if frame.UpdateAnchors then
            -- Blizzard re-anchors + resets bgTexture in UpdateAnchors on every
            -- options/CVar update, so re-apply ours right after it runs
        ...
        if cb then
            -- No ApplyStyleAndAnchoring hook and no manual positioning -- the
            -- cast bar stays where Blizzard puts it (so it's always visible).
            -- We only recolor by spell and hide friendly casts.
            cb:HookScript("OnShow", CastBarVisuals)

            cb:HookScript("OnUpdate", function(self)
                local parent = self:GetParent():GetParent() -- castBar -> CastBarsContainer -> UnitFrame
    ...
    -- hide level (anchoring/expanding of the healthbar moved into EvolveUI_UpdateAnchors)
