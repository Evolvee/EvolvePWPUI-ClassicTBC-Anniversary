...
BigAuras.anchors2frames = {
    ...
    ["party1"] = {
        ["Blizzard"] = "PartyFrame.MemberFrame1.Portrait",
    ...
    ["party2"] = {
        ["Blizzard"] = "PartyFrame.MemberFrame2.Portrait",
    ...
    ["party3"] = {
        ["Blizzard"] = "PartyFrame.MemberFrame3.Portrait",
    ...
    ["party4"] = {
        ["Blizzard"] = "PartyFrame.MemberFrame4.Portrait",
    ...
    ["partypet1"] = {
        ["Blizzard"] = "PartyFrame.MemberFrame1.PetFrame.Portrait",
        ["Perl"] = nil,
        ["XPerl"] = nil,
        ["ElvUI"] = nil,
    ...
    ["partypet2"] = {
        ["Blizzard"] = "PartyFrame.MemberFrame2.PetFrame.Portrait",
        ["Perl"] = nil,
        ["XPerl"] = nil,
        ["ElvUI"] = nil,
        ["ShadowedUnitFrames"] = nil
    ...
    ["partypet3"] = {
        ["Blizzard"] = "PartyFrame.MemberFrame3.PetFrame.Portrait",
        ["Perl"] = nil,
        ["XPerl"] = nil,
        ["ElvUI"] = nil,
        ["ShadowedUnitFrames"] = nil
    ...
    ["partypet4"] = {
        ["Blizzard"] = "PartyFrame.MemberFrame4.PetFrame.Portrait",
        ["Perl"] = nil,
        ["XPerl"] = nil,
        ["ElvUI"] = nil,
        ["ShadowedUnitFrames"] = nil
...
local function UPDATE_BATTLEFIELD_STATUS(_, index)
    local status, _, _, _, _, teamSize = GetBattlefieldStatus(index)
...
function BigAuras:getOrCreate(unit)
    ...
    if not self.frames[unit] then
        ...
        if not frame.CircleMask then
            frame.CircleMask = frame:CreateMaskTexture()
            frame.CircleMask:SetTexture("Interface/CHARACTERFRAME/TempPortraitAlphaMask", "CLAMPTOBLACKADDITIVE", "CLAMPTOBLACKADDITIVE")
            frame.CircleMask:SetAllPoints(frame.Icon)
        end
    ...
    if frame.db.unlock or parent == UIParent then
        ...
        if portrait and self.db.profile.anchor == "Blizzard" then
			...
			-- adjusted for the new slimmer Evolve PWP UI frames
                --frame:SetPoint("TOPLEFT", portrait, "TOPLEFT", 2, -2)
                --frame:SetPoint("BOTTOMRIGHT", portrait, "BOTTOMRIGHT", -2, 2)
                frame:SetAllPoints(portrait)
                frame.Cooldown:ClearAllPoints()
                frame.Cooldown:SetAllPoints(frame)
            end
            ...
            frame:SetScale(parent:GetScale() * 1.05)
            frame:ClearAllPoints()
...
function BigAuras:GetParent(unit)
    ...
    if not self.portraitSupport[self.db.profile.anchor] and parent then
        ...
        if self.db.profile.anchor == "Blizzard" and unit:match("^party") and _G.PartyFrame then
            for memberFrame in _G.PartyFrame.PartyMemberFramePool:EnumerateActive() do
                if memberFrame.unit == unit then
                    portrait = memberFrame.Portrait
                    break
                end
            end
        end
        ...
        if not portrait then
            local frame = _G
            for key in string.gmatch(parent, "[^%.]+") do
                frame = frame and frame[key]
            end
            portrait = frame
        end
...
function BigAuras:support_s_Arena()
    if C_AddOns.IsAddOnLoaded("sArena") and sArenaIconStyle then
