...
local ATTversion = C_AddOns.GetAddOnMetadata("ATT", "Version")
local db
local selectedDB
...
function ATT:ProcessInspect(unit, guid, specInspect, gearInspect)
    if unit and specInspect and guid then
        dbInspect[guid] = {}
        local isInspect = (guid ~= PlayerGUID) and true
        for i = 1, 3 do
            for j = 1, 31 do
        ...
        if gearInspect then
            ...
            for k = 1, 14 do
                ...
                if itemID then
                    if ATTdbs.dbTrinketsMerge and ATTdbs.dbTrinketsMerge[itemID] then itemID = ATTdbs.dbTrinketsMerge[itemID] end
...
function ATT:ProcessInspectQueue()
    ...
    for i, guid in pairs(inspect_queue) do
        ...
        if not unit or (unit and (not UnitIsConnected(unit) or (self.insStart[guid]and self.insStart[guid] + 120 < cTime))) then
            self:DequeueInspectByGUID(guid)
            break
        end  

        if not InCombatLockdown() and unit and not (InspectFrame and InspectFrame:IsShown()) and CheckInteractDistance(unit, 1) and CanInspect(unit) and (not self.insTime[guid] or self.insTime[guid] + 2 < cTime) then
            self.lastIns = guid
...
function ATT:CheckBlizzFrames()
    ...
    if ATT.inRaid then
        ...
        if compact then
            ...
            frametype = "PartyFrame.MemberFrame%d"
        end
    end
...
function ATT:FindFrames()
    ...
    if isGrouped then
        ...
        elseif frametype == "PartyFrame.MemberFrame%d" then
...
function ATT:TrackCooldown(anchor, ability, SentID, unit, guid, flag)
    ...
    for k, icon in ipairs(anchor.icons) do
        ...
        -- Cooldown Share
        if ShareCD[ability] and ShareCD[ability][icon.abilityID] and icon.inUse and not icon.active then
            icon.Start(ShareCD[ability][icon.abilityID] == 1 and icon.cooldown or ShareCD[ability][icon.abilityID], 1, rate)
        end
        ...
        if cooldownResetters[ability] then
            if type(cooldownResetters[ability]) == "table" and (cooldownResetters[ability][icon.abilityID] or cooldownResetters[ability]["ALL"]) and not icon.excluded and icon.inUse then
                local validReset = cooldownResetters[ability][icon.abilityID] == 1 or (dbInspect[guid] and dbInspect[guid][cooldownResetters[ability][icon.abilityID]])
...
function ATT:COMBAT_LOG_EVENT_UNFILTERED()
    ...
    if (event == "SPELL_SUMMON" or event == "SPELL_CAST_SUCCESS" or event == "SPELL_MISSED" or event == "SPELL_DAMAGE") then
        self:StartCooldown(spellName, unit, SentID, dbAuraRemoved[spellName] and "AuraOn")
    end
    
    if event == "SPELL_AURA_APPLIED" and auraType == "BUFF" then
    ...
    if db.glow and (event == "SPELL_AURA_APPLIED" or event == "SPELL_AURA_REMOVED") and auraType == "BUFF" then
        local dest = unitDest or unit
...
ATT.MergeCache = ATT.MergeCache or {}

function ATT:MergeTable(class, category, isAnchor)
    local cacheKey = class .. category .. tostring(isAnchor)
    if ATT.MergeCache[cacheKey] then return ATT.MergeCache[cacheKey] end

    local newdbSpells = {}

    if not db.alertCDtext then db.alertCDtext = {} end
    ...
    ATT.MergeCache[cacheKey] = newdbSpells
    return newdbSpells
end
...
function ATT:FindFrameType()
    ...
    for x, v in pairs(customframes) do
        ...
        if (_G[checkframe] or C_AddOns.IsAddOnLoaded(v.cname)) then
